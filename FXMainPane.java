...
public class FXMainPane extends VBox {
	...
	FXMainPane() {
		...
		displayTownsButton.setOnAction(event -> {
			ArrayList<String> towns = graph.vertexSet();
			String result = "";
		...
		displayRoadsButton.setOnAction(event -> {
			ArrayList<String> roads = graph.edgeSet();
			String result = "";
		...
		addRoadButton.setOnAction(event -> {
			...
			Town town2;
			try {
				town1 = graph.getVertex(new Town(addSourceTownComboBox.getValue().toString()));
				town2 = graph.getVertex(new Town(addDestTownComboBox.getValue().toString()));
			...
			if (weight < 0) {
				...
				graph.addEdge(new Town(town1.getName()), new Town(town2.getName()), weight, name);
		...
		addTownButton.setOnAction(event -> {
			...
			else if (graph.addVertex(new Town(townName))){
				updateComboBoxes();
		...
		findConnectionButton.setOnAction(event -> {
			...
			String result = "";
			try {
				town1 = graph.getVertex(new Town(sourceConnectionComboBox.getValue().toString()));
				town2 = graph.getVertex(new Town(destConnectionComboBox.getValue().toString()));
			} catch (NullPointerException e) {
				town1 = town2 = null;
			}
			findConnectionTextArea.setText("");
			ArrayList<String> path = graph.dijkstra(town1, town2);
	...
	public void updateComboBoxes() {
		ArrayList<String> townList = graph.vertexSet();
